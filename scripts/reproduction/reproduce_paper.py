        ...
        for mode in ("step", "ramp"):
            ...
            _run_logged(
                [
                    sys.executable,
                    "-m",
                    "scripts.data.build_receiver_fault_cases",
    ...
    for name, case_root in cases.items():
        ...
        _run_logged(
            [
                sys.executable,
                "-m",
                "scripts.experiments.run_ci_baseline",
                "--epoch-policy", "all_sources",
                "--case-root", str(case_root),
                "--output", str(report),
                "--trajectory-output", str(trajectory),
...
def _revision_diagnostics(
    ...
    _run_logged(
        [
            sys.executable,
            "-m",
            "scripts.analysis.analyze_revision_diagnostics",
...
def _statistics(output: Path, trials: int, overwrite: bool) -> None:
    jobs = (
        (
            "paired",
            "scripts.analysis.analyze_paired_statistics",
            [
        ...
        (
            "ci",
            "scripts.analysis.analyze_ci_statistics",
            [
                "--natural-root", str(output / "natural"),
                "--fault-root", str(output / "fault"),
        ...
        (
            "crossrun",
            "scripts.analysis.analyze_crossrun_statistics",
            [
    ...
    for label, module, arguments, expected in jobs:
        _run_logged(
    ...
    _run_logged(
        [
            sys.executable,
            "-m",
            "scripts.analysis.analyze_priority_ablation",
    ...
    _run_logged(
        [
            sys.executable,
            "-m",
            "scripts.analysis.analyze_leave_one_panel_out",
    ...
    if "runtime" in requested:
        ...
        _run_logged(
            [
                sys.executable,
                "-m",
                "scripts.analysis.summarize_runtime",
    ...
    if "verify" in requested:
        ...
        _run_logged(
            [
                sys.executable,
                "-m",
                "scripts.reproduction.verify_reproduction",
